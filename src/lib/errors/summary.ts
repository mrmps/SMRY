...
export function parseSummaryError(errorInput: string | Error): SummaryError | null {
  ...
  try {
    ...
    if (typeof parsed === 'object' && parsed !== null) {
      ...
      if (typeof parsedObj.code === 'string' && parsedObj.code in SUMMARY_ERRORS) {
        const code = parsedObj.code as SummaryErrorCode;
        return {
          ...SUMMARY_ERRORS[code],
          retryAfter: typeof parsedObj.retryAfter === 'number' ? parsedObj.retryAfter : undefined,
          usage: typeof parsedObj.usage === 'number' ? parsedObj.usage : undefined,
          limit: typeof parsedObj.limit === 'number' ? parsedObj.limit : undefined,
        };
      }
      // Legacy format: { error: "message" }
      if (typeof parsedObj.error === 'string') {
        return matchErrorMessage(parsedObj.error);
      }
    }
  } catch {

...
function extractStatusFromError(errorMsg: string): number | undefined {
  ...
  const match = /\((\d{3})\)|HTTP\s*(\d{3})|error\s*(\d{3})|status\s*(\d{3})/i.exec(errorMsg);
  if (match) {
...
const DiffbotAuthorSchema = z.object({
...
const DiffbotArticleObjectSchema = z.object({
  ...
  authors: z.array(DiffbotAuthorSchema).optional(),
  ...
  for (const script of Array.from(scripts)) {
    try {
      ...
      if (typeof json === 'object' && json !== null) {
        ...
        if (typeof jsonObj.datePublished === 'string') return jsonObj.datePublished;
        if (typeof jsonObj.dateCreated === 'string') return jsonObj.dateCreated;
        ...
        if (Array.isArray(graph)) {
          for (const item of graph) {
            if (typeof item === 'object' && item !== null) {
              ...
              if (typeof itemObj.datePublished === 'string') return itemObj.datePublished;
              if (typeof itemObj.dateCreated === 'string') return itemObj.dateCreated;
            }
          }
        }
      }
...
function extractWithReadability(html: string, url: string, debugContext: DebugContext): DiffbotArticle | null {
  ...
  try {
    ...
    if (url.includes('web.archive.org')) {
      const match = /web\.archive\.org\/web\/\d+\/(.+)/.exec(url);
      if (match) {
    ...
    for (const selector of contentSelectors) {
      ...
      if (container?.textContent && container.textContent.length > 500) {
 ...
 */
export function fetchArticleWithDiffbot(url: string, source = 'smry-slow'): ResultAsync<DiffbotArticle, AppError> {
  ...
  return ResultAsync.fromPromise(
    fetchDiffbotArticleInternal(url, debugContext),
    (error) => {
...
/**
 * Internal async function to fetch article from Diffbot API
 * Separated from fetchArticleWithDiffbot to avoid async Promise executor
 */
async function fetchDiffbotArticleInternal(url: string, debugContext: DebugContext): Promise<DiffbotArticle> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Diffbot HTTP error: ${response.status}`);
    }
    ...
    if (!responseValidation.success) {
      ...
      const receivedKeys = typeof rawData === 'object' && rawData !== null ? Object.keys(rawData) : [];
      logger.error({
        url,
        validationError: validationError.toString(),
        receivedKeys
      ...
      throw new Error(`Invalid Diffbot API response structure: ${validationError.toString()}`);
    }
    ...
    addDebugStep(debugContext, 'diffbot_response', 'info', 'Received Diffbot API response', {
      diffbotResponse: {
        ...
        hasText: !!data?.text,
        hasTitle: !!data?.title,
        hasDom: !!(data?.objects?.[0]?.dom ?? data?.dom),
    ...
    if (data?.objects && Array.isArray(data.objects) && data.objects.length > 0) {
      ...
      if (obj.html && obj.text && obj.title && obj.text.length > 100) {
        ...
        let extractedImage: string | null | undefined = null;

        if (obj.images && obj.images.length > 0) {
             const img = obj.images.find((i) => i.url && i.primary) ?? obj.images.find((i) => i.url);
        ...
        if ((!extractedDate || !extractedImage) && (obj.dom ?? domForFallback)) {
           try {
             ...
             extractedImage ??= extractImageFromDom(doc);
           } catch {
             // Ignore errors
           }
        }
        ...
        const completeArticle: DiffbotArticle = {
          ...
          byline: obj.author ?? (obj.authors && obj.authors.length > 0 ? obj.authors.map((a) => a.name).join(', ') : null),
    ...
    // Try old API format
      ...
      const dom = data.dom ?? domForFallback;
      let extractedDate = data.date;
      let extractedImage: string | null | undefined = null;
      if (data.media && data.media.length > 0) {
          const img = data.media.find((m) => m.type === 'image' && m.link);
      ...
      articleData = {
        ...
        publishedTime: extractedDate,
        image: extractedImage,
        htmlContent: dom ?? undefined, // Original page HTML (full DOM)
      };
      ...
      if (!articleValidation.success) {
        ...
        // Don't return, let it fall through to check for DOM fallback
      } else {
        const validatedArticle = articleValidation.data;
    ...
    throw new Error(`Could not extract article content for URL: ${url}`);

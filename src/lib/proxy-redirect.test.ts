...
describe("proxy redirect helpers", () => {
  ...
  describe("buildProxyRedirectPath", () => {
    describe("app routes (should return null)", () => {
      it("returns null for root", () => {
        expect(buildProxyRedirectPath("/", "")).toBe(null);
      });

      it("returns null for /proxy", () => {
        expect(buildProxyRedirectPath("/proxy", "")).toBe(null);
      });

      it("returns null for /proxy with query params", () => {
        expect(
          buildProxyRedirectPath("/proxy", "?url=https://example.com")
        ).toBe(null);
      });

      it("returns null for /pricing", () => {
        expect(buildProxyRedirectPath("/pricing", "")).toBe(null);
      });

      it("returns null for /api/article", () => {
        expect(buildProxyRedirectPath("/api/article", "")).toBe(null);
      });
    ...
    describe("basic URLs without query params", () => {
      it("builds proxy URL for bare domain", () => {
        const result = buildProxyRedirectPath("/nytimes.com", "");
        expect(result).toBe("/proxy?url=https%3A%2F%2Fnytimes.com");
      });

      it("builds proxy URL for bare domain with path", () => {
        const result = buildProxyRedirectPath(
          "/www.nytimes.com/2025/article",
          "",
        );
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fwww.nytimes.com%2F2025%2Farticle"
        );
      });
    ...
    it("builds proxy URL for full https URL", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fwww.nytimes.com%2Farticle"
        );
    });
    ...
    it("builds proxy URL for collapsed protocol URL", () => {
      ...
      it("builds proxy URL for URL ending in .html", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fwww.nytimes.com%2F2025%2F12%2F08%2Farticle.html"
        );
    });
    });
    ...
    describe("URLs with external query params (belong to the target URL)", () => {
      it("includes external query params in the url param", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Ffoo.com%2Farticle%3Fx%3D1"
        );
      });
      ...
      it("handles bare domain with query params", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fexample.com%3Fref%3Dsocial"
        );
      });
    });
    ...
    describe("SMRY UI params (should be preserved separately)", () => {
      it("separates sidebar param from external URL", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Ffoo.com%2Farticle&sidebar=open"
        );
      });
      ...
      it("separates tab param from external URL", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Ffoo.com%2Farticle&tab=summary"
        );
      });
      ...
      it("separates source param from external URL", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Ffoo.com%2Farticle&source=jina.ai"
        );
      });
    });
    ...
    describe("edge cases", () => {
      ...
      it("handles domains that look like app routes", () => {
        ...
        const result = buildProxyRedirectPath("/proxy.com", "");
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fproxy.com"
        );
      });
      ...
      it("handles /api.example.com (not /api route)", () => {
        ...
        expect(result).toBe(
          "/proxy?url=https%3A%2F%2Fapi.example.com"
        );
    });

...
function GitHubStarsButton({
  ...
  const { data: stars, isLoading } = useQuery<number>({
    ...
    queryFn: async (): Promise<number> => {
      ...
      if (cached) {
        ...
        if (Date.now() - parsed.timestamp < fiveDays) {
          return parsed.stars;
        }
      }
  ...
  return (
    <motion.a
      ...
      {...props}
    >
      <svg role="img" viewBox="0 0 24 24" fill="currentColor" className="text-muted-foreground">

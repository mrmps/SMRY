...
export interface MenuDockItem {
  ...
  onClick?: () => void;
  badge?: React.ReactNode; // Support for word count or other badges
  badgeVariant?: 'dot' | 'count' | 'subtle'; // How to display the badge
  value?: string; // Value for tracking active state
}
...
export interface MenuDockProps {
  ...
  showLabels?: boolean;
  animated?: boolean;
  activeValue?: string; // Current active item value
}
...
export const MenuDock: React.FC<MenuDockProps> = ({
  ...
  // Find active index based on activeValue
  useEffect(() => {
    if (activeValue) {
  ...
  return (
    ...
    >
      {finalItems.map((item, index) => {
        const isActive = activeValue ? item.value === activeValue : index === activeIndex;
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* Badge as overlay - space-efficient solution */}
              {item.badge && (
                <div className={cn(
                  ...
                  item.badgeVariant === 'subtle' && 'bg-muted text-muted-foreground'
                )}>
                  {item.badgeVariant !== 'dot' && item.badge}
                </div>
              )}
            </div>
            {showLabels && (

...
function AnalyticsDashboardContent() {
  ...
  const changeTimeRange = useCallback((newRange: string) => {
    ...
    if (outcomeFilter) params.set("outcome", outcomeFilter);
    if (debouncedUrlSearch) params.set("urlSearch", debouncedUrlSearch);
    routerHistory.push(`/admin?${params.toString()}`);
  }, [routerHistory, hostnameFilter, sourceFilter, outcomeFilter, debouncedUrlSearch]);
            ...
            <LineChart data={sourceErrorRateChartData}>
              ...
              <Tooltip
                ...
                formatter={(value, name, props) => {
                  ...
                  const requests = payload[`${name}_requests`];
                  return [`${String(value ?? 0)}% (${String(requests ?? 0)} requests)`, name];
                }}
              />
              <Legend />
              <Line
...
// SeverityBadge component - currently unused but may be needed for future error displays
...
// Default export with Suspense boundary to keep loading states consistent.

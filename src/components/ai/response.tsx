...
function parseIncompleteMarkdown(text: string): string {
  ...
  if (inlineCodeMatch) {
    ...
    // Count complete code blocks for logic
    result.match(codeBlockPattern);
...
const components: Options['components'] = {
  ...
  code: ({ node, className, ...props }) => {
    ...
    const inline = position?.position?.start.line === position?.position?.end.line;
    if (!inline) {
  ...
  pre: ({ node: _node, className, children }) => {
    const nodeWithProps = _node as { properties?: { className?: string } } | undefined;
    let language = 'javascript';
    if (typeof nodeWithProps?.properties?.className === 'string') {
      language = nodeWithProps.properties.className.replace('language-', '');
    }

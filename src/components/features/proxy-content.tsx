...
import type { ViewMode } from "@/routes/shared/proxy-route";
...
export function ProxyContent({ url }: ProxyContentProps) {
  ...
  // Use TanStack Router's built-in URL state management
  const search = useSearch({ strict: false });
  const navigate = useNavigate();

  // Get values with defaults
  const source: Source = search.source ?? "smry-fast";
  const viewMode: ViewMode = search.view ?? "markdown";
  const sidebarOpen: boolean = search.sidebar ?? false;
  ...
  useEffect(() => {
    ...
    if (firstSuccess && savedToHistoryRef.current !== url) {
      ...
      void addArticleToHistory(url, title);

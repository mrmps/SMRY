...
export function HomeContent() {
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      void navigate({
        to: "/proxy",
        search: { url: parsed.url, source: "smry-fast", view: "markdown", sidebar: false },
      });
    } catch (error) {
      const message =
      ...
      <main className="flex min-h-screen flex-col items-center bg-background p-4 pt-20 text-foreground sm:pt-24 md:p-24 pb-24 lg:pb-4">
        <div className="z-10 mx-auto flex w-full max-w-lg flex-col items-center justify-center sm:mt-16">
          ...
          <p className="mt-2 text-center text-lg text-muted-foreground">
            {t("tagline")}{" "}
            <Link
              to="/proxy?url=https://www.theatlantic.com/technology/archive/2017/11/the-big-unanswered-questions-about-paywalls/547091"

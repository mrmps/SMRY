...
export function ExportHighlights({
  ...
  // Use all highlights if provided, otherwise single article
  ...
  const generateObsidianMarkdown = useCallback((): string => {
    ...
    for (const article of highlightsToExport) {
      ...
      lines.push(`# ${article.articleTitle || "Untitled Article"}`);
      lines.push("");
      lines.push(`[Original Source](${article.articleUrl})`);
      lines.push("");
      lines.push("## Highlights");
      lines.push("");

      for (const hl of article.highlights) {
        const colorTag = `#highlight/${hl.color}`;
        lines.push(`> [!quote] ${colorTag}`);
        lines.push(`> ${hl.text}`);
        if (hl.note) {
          lines.push(`>`);
          lines.push(`> **Note:** ${hl.note}`);
  ...
  // Generate Roam Research markdown
  ...
  const generateMarkdown = useCallback((): string => {
    const lines: string[] = [];
    lines.push("# Highlights from smry.ai");
    lines.push("");
    ...
    for (const article of highlightsToExport) {
      ...
      lines.push(`## ${article.articleTitle || "Untitled Article"}`);
      lines.push(`[${article.articleUrl}](${article.articleUrl})`);
      lines.push("");
  ...
  const handleCopy = useCallback(async (format: ExportFormat) => {
    ...
    switch (format) {
      case "notion":
        content = generateNotionMarkdown();
        break;
      case "obsidian":
        content = generateObsidianMarkdown();
        break;
      case "roam":
        content = generateRoamMarkdown();
        break;
      case "markdown":
  ...
  const exportOptions: {
  ...
  }[] = [
    {
      format: "notion",
      name: "Notion",
      icon: <NotionIcon className="size-5" />,
      description: "Paste directly into Notion",
    },
    {
      format: "obsidian",
      name: "Obsidian",
      icon: <ObsidianIcon className="size-5" />,
      description: "With frontmatter and callouts",
    },
    {
      format: "roam",
      name: "Roam Research",
      icon: <RoamIcon className="size-5" />,
      description: "With [[page links]] format",
    },
    {
      format: "markdown",
      name: "Markdown",

...
function TranscriptViewerAudio({
  ...
  const { audioProps, audioRef } = useTranscriptViewerContext()
  // Destructure ref out to avoid "Cannot access refs during render" error.
  // The ref is applied separately via the explicit ref prop.
  const { ref: _ref, ...restAudioProps } = audioProps
  return (
    <audio
...
export type { CombinedAlignment as TranscriptAlignment }
